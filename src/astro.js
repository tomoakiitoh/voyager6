...
/* ---- ΔT (地球時 TT − 世界時 UT) ----
   天体の位置は TT で、地球の自転は UT で決まる。両者は地球の自転むらのぶんだけずれ、
   2026年で約70秒ある。星図では 70秒 = 空が 0.3′ 回るだけで問題にならないが、
   **日食の影では効く**: 影は地表を秒速1km前後で走るので、70秒 = 約70km。
   実測でも、ΔT を入れないと 2026-08-12 の本影が NASA の値から 89km ずれ、
   入れると 38km に縮んだ。

   式は Espenak & Meeus (NASA の日食ページが使っているもの) の区分多項式。
   1900〜2150年を覆う。返り値は秒。 */
function deltaT(jd) {
  // 年 (小数)。月の中央を使う近似で十分 (ΔT の傾きは年0.3秒程度)
  const y = 2000 + (jd - 2451545.0) / 365.25;
  let u, t;
  if (y < 1920)      { t = y - 1900; return -2.79 + 1.494119 * t - 0.0598939 * t * t
  ...
  if (y < 2005)      { t = y - 2000; return 63.86 + 0.3345 * t - 0.060374 * t * t + 0.0017275 * t ** 3
...
/** 世界時のユリウス日 → 地球時のユリウス日。天体の位置計算に渡すのはこちら。 */
function jdTT(jdUT) { return jdUT + deltaT(jdUT) / 86400; }

/* ---- 月の位置 (ELP2000-82B 主要項, Meeus 第47章) ----
   既存の moonPosition() は Schlyter 系列で、Meeus の検証例に対し黄経 -0.9′・黄緯 -0.6′。
   星図に描くぶんには足りるが、**食の幾何には足りない**: 0.9′ の角度誤差は月までの距離で
   地表に投影すると約120km で、本影の幅 (100〜270km) と同じ桁になる = 影ひとつぶんずれる。
   こちらは Meeus 表47.A/47.B の主要項 (経度・距離 60項 / 緯度 60項) で、黄経の精度は約10″
   = 地表で約19km。本影幅の1割程度に収まる。

   引数はすべて度。T は J2000 からのユリウス世紀。
   係数は Σl・Σb が 1e-6 度、Σr が 1e-3 km。 */

// [D, M, M', F, l(1e-6°), r(1e-3 km)]
const ELP_LR = [
...
function moonPositionELP(jd) {
  ...
  return { lon, lat, dist, r: dist / 6378.14 };
}
