...
def main() -> int:
    ...
    # 見出し語はサイトの顔 (<title>・h1) と同じ「今日の星空」に揃える (Ver.1.1)
    dr.text((72, 320), "今日の星空を、その場所で。", font=load_font(26), fill=(160, 180, 215))
    # 文言はサイトの現状に合わせる (Ver.1.0)。**盤面に食い込ませないこと** —
    # 円の左端は cx-R = 580px なので、x=72 から描いて終端が 560 を超えたら入らない。
    # 20px の日本語は1文字20pxなので、全角22文字ぶんが上限の目安。
    # (「望遠鏡ファインダー・…・探査機・日月食」まで入れると 620px で盤面に重なる)
    dr.text((72, 366), "星座早見・望遠鏡・太陽系3D・探査機・日月食",
